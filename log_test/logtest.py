    ...
    def generateData(self):
        ...
        while (True):
            ...
            if self.__generating:
                ...
                # newData = logtest.MainWindow.vpa_value_processing()
                # newData = vpa_list
...
class MplCanvas(FigureCanvas):
    ...
    def plot(self, datax, datay):
        ...
        if self.curveObj is None:
            ...
            self.curveObj.set_data(np.array(datax), np.array(datay))  #设置曲线数据

            # update limit of X axis,to make sure it can move

            self.ax.set_xlim(datax[0], datax[-1])  #设置刻度范围

        ticklabels = self.ax.xaxis.get_ticklabels()

        for tick in ticklabels:
            tick.set_rotation(25) #把tick轴上的label旋转25度,以免太密集时有重叠
